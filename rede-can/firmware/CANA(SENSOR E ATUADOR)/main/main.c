...
void CAN_Task(void *pvParameters)
{
    ...
    while(1)
    {
        ...
        // valor_velocidade_pot guarda SEMPRE a posição física pura (0 a 1000)
        ...
        // A variação é comparada com o ÚLTIMO VALOR ACEITO
        ...
        if (diferenca_movimento_fisco >= threshold_mudanca) {
            ...
            // ATUALIZA A LINHA DE BASE
            ultima_leitura_real_pot = valor_velocidade_pot; 
            
            printf("[EVENTO HARDWARE] Potenciômetro assumiu: %u\n", g_velocidade_sistema);
        }
        // Garante o zero absoluto se o botão for levado ao batente inicial
            ...
            ultima_leitura_real_pot = 0; // Atualiza a referência no zero
            printf("[EVENTO HARDWARE] Zerado pelo potenciômetro\n");
        }

        // ------------------------------------------------------
        // TRANSMISSÃO CÍCLICA FIXA - ID 0x4D2
        // ------------------------------------------------------
        frame_tx.can_id = 0x4D2;
        frame_tx.can_dlc = 8;
        memset(frame_tx.data, 0, 8);
        
        // Bytes 0 e 1: Velocidade consolidada do sistema (vinda de quem ganhou a concorrência)
        frame_tx.data[0] = g_velocidade_sistema & 0xFF;        
        frame_tx.data[1] = (g_velocidade_sistema >> 8) & 0xFF; 
        
        // Bytes 2 e 3: Envia a posição REAL e pura do hardware do potenciômetro (para cálculo da tensão no CANB)
        frame_tx.data[2] = valor_velocidade_pot & 0xFF;
        frame_tx.data[3] = (valor_velocidade_pot >> 8) & 0xFF;
